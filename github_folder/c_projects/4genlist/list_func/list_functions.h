...
/** 
 * @brief Predicate function returns a none zero value if predicate
 * 			holds for element.
 ...
 * @details find the element for which the predicate reuturns a non
 * 			zero value in the half open range [begin..end)
 ...
 * @return an iterator pointing to the first node with data found or
 *		   to _end if not
 */
 ...
 *			and insert them before _dest O(?)
 ...
 * @details Merge elements from two sub lists defined by
 * 			[_firstbegin.._firstEnd) and [_secondBegin.._secondEnd)
 * 			in optionaly sorted order using _less function.
 ...
 * 			O(?)
 */
ListItr ListItrMerge(ListItr _destBegin, ListItr _firstBegin,
	ListItr _firstEnd, ListItr _secondBegin, ListItr _secondEnd,
	LessFunction _less);

/** 
 * @brief Removes a sublist [_begin.._end) from a list and creates a
 * 		  new list with all removed items.
 * 		  O(?)
 */
List* ListItrCut(ListItr _begin, ListItr _end);

/** 
 * @brief Removes all duplicate elements from a sorted sublist
 * 		  [_begin.._end) and rertun a new list with all removed items.
 * 		  O(?)
