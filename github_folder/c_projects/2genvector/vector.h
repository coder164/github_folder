...
* arguments:	_initialCapacity: the initial number of elements that
				...
				_blockSize: the size of extention block if the vector
					is full. also the size to be removed in shrinkage
					of the vector.
...
* arguments:	_vector: the vector to be destroyed
				_elementDestroy: A function pointer to be used to
					destroy all of the elements of vector. it can also
					be null if no such destroy is required.
...
* errors:		- VECTOR_UNITIALIZED_ERROR:
 					- if _vector is invalid.
					- if _item is invalid.
				- VECTOR_REALOCATION_ERROR
					- if realoc failed.
				- VECTOR_OVERFLOW_ERROR
					- if the blocksize defined as '0' and the _vector
					  is full.*/
...
/*description:	get the value of element inside the vector by index and
				save it in '_pValue'
* arguments:	_vector: Vector
...
* errors:		VECTOR_UNITIALIZED_ERROR
					- if vector address is invalid
				VECTOR_WRONG_INDEX
					- if _index is bigger then the number of items.*/
...
* arguments:	_vector - vector to be iterated over.
				_action - generic function to be invoked at each
						  iteration.
				_context - the pointer to be sent to the _action
						   function.
...
* equivalent to:	for(i = 0; i < VectorSize(v); ++i){
					...
					break;} return i}
* errors:		0 - if _vector is invalid
				  - if _action is invalid.*/
